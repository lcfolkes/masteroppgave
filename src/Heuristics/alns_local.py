...
class ALNS():
                ...
                if solution.hash_key in visited_hash_keys:
                    ...
                    if self._accept(current_obj_val, best_obj_val, temperature):
                        ...
                        if obj_val >= current_obj_val:
                            ...
                            print("\n###### Local Search ######\n")
                            for _ in range(10):
                                mutated_obj_val_true, mutated_obj_val, mutated_solution = self._mutate_solution(
                                    repaired_solution=repair.repaired_solution,
                                    feasibility_checker=repair.feasibility_checker,
                                    current_solution=current_solution)

                                visited_hash_keys.add(mutated_solution.hash_key)
                                heuristic_obj_vals.append(mutated_obj_val)
                                true_obj_vals.append(mutated_obj_val_true)

                                if mutated_obj_val > obj_val:
                                    print("Local search successful!")
                                    obj_val = mutated_obj_val
                                    true_obj_val = mutated_obj_val_true
    ...
    def _mutate_solution(self, repaired_solution, feasibility_checker, current_solution):

        local_search_operator = self._get_local_search_operator(repaired_solution, feasibility_checker)
        mutated_solution = local_search_operator.mutated_solution
        solution = copy.deepcopy(current_solution)
        solution.rebuild(mutated_solution)
        true_obj_val, obj_val = solution.get_obj_val(both=True)
        print(f"mutated_solution: {solution} \ntrue_obj_val: {true_obj_val} \nobj_val: {obj_val}")

        return true_obj_val, obj_val, solution
