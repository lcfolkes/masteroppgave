...
class Destroy(ABC):
    def __init__(self, solution, num_first_stage_tasks, neighborhood_size=2):
        ...
        self.first_stage_solution, self.removed_moves = get_first_stage_solution_and_removed_moves(solution,
                                                                                                   num_first_stage_tasks)
...
class RandomRemoval(Destroy):
    ...
    def _destroy(self):
        ...
        for move in self.removed_moves:
            if move.is_charging_move:
                move.end_node.remove_car()
                print("Remove MoveID", move.car_move_id)
                print("Endnode", move.end_node.node_id)
                print("Endnode", move.end_node)
                print("Num charging", move.end_node.num_charging)
                return solution
    ...
    def _destroy(self):
        ...
        while n_size > 0 and len(obj_val_list) > 0:
            ...
            try:
                ...
                removed_car_moves.append(first_stage_solution_list[idx])
            except:
