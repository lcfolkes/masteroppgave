...
def check_all_charging_moves_completed(num_scenarios: int, employees: [Employee], first_stage: bool,
                                       parking_nodes: [ParkingNode]) -> bool:
    """
    Checks if all charging moves are completed, that is, that all the number of charging moves assigned to the
    employees equals the total number of cars in need of charging
    :param num_scenarios:
    :param employees:
    :param first_stage:
    :param parking_nodes:
    :return: True if all charging moves are completed
    """
...
def get_assigned_car_moves(employees: [Employee], scenario: int = None) -> [CarMove]:
    """
    Returns a list of all car moves that are assigned to the list of employees taken as input. If a scenario is
    specified, only car the car moves from that scenario are returned.
    :param employees: A list of employee objects
    :param scenario:
    :return: A list of car all car moves assigned to the employees
    """
    car_moves = []
    if scenario is None:
        for employee in employees:
...
def get_best_car_move(parking_nodes: [ParkingNode], employees: [Employee], car_moves: [CarMove], first_stage: bool,
                      prioritize_charging: bool, num_scenarios: int) -> CarMove:
    """
    Returns the best car move based on the parking nodes, employees and car moves given as input. "Best" here means the
    car move that yields the highest increase in objective value.
    :param parking_nodes: A list of parking node objects
    :param employees: A list of employee objects
    :param car_moves: A list of car move objects
    :param first_stage: Boolean indicating whether one wants the best car move from the first stage
    :param prioritize_charging: Boolean indicating whether charging should be prioritize
    :param num_scenarios:
    :return:
    """
    # FIRST STAGE
    if first_stage:
        ...
        # print(f"best_car_move: {best_car_move_first_stage.car_move_id},
        # {best_car_move_first_stage.start_node.node_id} --> {best_car_move_first_stage.end_node.node_id}")
...
def get_best_employee(parking_moves: [CarMove], employees: [Employee], best_car_move: CarMove, first_stage: bool,
                      num_scenarios: int, world_instance: World, prioritize_charging: bool, charging_moves: [CarMove],
                      charging_moves_second_stage: [CarMove], parking_moves_second_stage: [CarMove]) -> Employee:
    """
    Returns the best employee based to perform the car move given as input
    :param parking_moves:
    :param employees:
    :param best_car_move:
    :param first_stage:
    ...
    :param prioritize_charging:
    :param charging_moves:
    :param charging_moves_second_stage:
...
def get_objective_function_val(parking_nodes: [ParkingNode], employees: [Employee], num_scenarios: int) -> float:
    ...
    :return: returns float of objective value for all car moves assigned to employees
    """
