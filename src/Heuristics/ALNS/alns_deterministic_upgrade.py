...
from Heuristics.DestroyAndRepairHeuristics.destroy import Destroy, RandomRemoval, WorstRemoval, ShawRemoval, \
	ChargeRemoval
...
class ALNS():
	...
	def __init__(self, filename, param=None, construction_heuristic=None, start_time=0):
		self.filename = filename
		self.is_deterministic = self._is_deterministic(filename)
		...
		if construction_heuristic is not None:
			self.solution = construction_heuristic
			self.received_ch = True
		else:
	...
	def run(self, run=0, verbose=False):
		...
		if self.is_deterministic:
			time_limit = HeuristicsConstants.DETERMINISTIC_TIME_LIMIT
		else:
			time_limit = HeuristicsConstants.TIME_LIMIT - self.start_time
		...
		iterations = [0]
		best_solution = (
		copy_solution_dict(self.solution.assigned_car_moves), true_obj_val, self.solution.num_charging_moves)
						...
						if round(candidate_obj_val, 2) > round(current_obj_val, 2):
							...
							if round(candidate_obj_val, 2) > round(best_obj_val, 2):
								...
								best_solution = (copy_solution_dict(self.solution.assigned_car_moves), true_obj_val,
												 self.solution.num_charging_moves)
								output_text += str(
		...
		if self._num_employees < 3:
			...
			operators = OrderedDict(
				{'random_greedy': 1.0, 'random_regret2': 1.0, 'random_regret3': 1.0, 'random_regret4': 1.0,
				 ...
				 'charge_greedy': 1.0, 'charge_regret2': 1.0, 'charge_regret3': 1.0, 'charge_regret4': 1.0,
				 'charge_charge': 1.0})
	...
	def _get_destroy_operator(self, solution, world_instance) -> \
		...
		neighborhood_size = random.uniform(HeuristicsConstants.DESTROY_REPAIR_FACTOR[0],
										   HeuristicsConstants.DESTROY_REPAIR_FACTOR[1])
	...
	def _is_deterministic(self, filename):
		filename_list = filename.split("-")
		if filename_list[1] == '1':
			return True
		else:
			return False
