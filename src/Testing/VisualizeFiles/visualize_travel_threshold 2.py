...
relocation_time_threshold_factor = 0.7
...
def plot_values_std(handling_times, parking_values=[], charging_values=[]):
	...
	for ax in g.axes.flat:
		for i, line in enumerate(ax.lines):
			...
			if i == 0:
				...
				ax.text(relocation_time_threshold_factor + 0.012, 0.022, percentage, **dict(size=10))
				...
				ax.text(relocation_time_threshold_factor + 0.012, 0.45, percentage, **dict(size=10))
				ax.fill_between(xs, 0, ys, facecolor=colors[i], alpha=0.1)
	...
	g._legend.remove()
	#ax.spines['top'].set_visible(False)
	g.set(xlabel='Fraction of longest relocation time $\\theta$')
	...
	plt.legend(labels=labels, loc='upper right', bbox_to_anchor=(1.3, 1.0))

	plt.show()
	g.savefig("car_moves_distribution.pdf", bbox_inches='tight')
...
def plot_travel_time_threshold():
	...
	ax1.set_xlabel('Relocation time threshold factor $\\theta$')
	...
	ax2.set_ylabel('Computational time (s)')  # we already handled the x-label with ax1
	ax2.plot(x, time, label="Computational time (s)", color="darkorange")
	...
	plt.xticks([0.1 * x for x in range(11)], [str(round(0.1*x, 1)) if (x%2==0) else "" for x in range(11)])
	plt.show()
	fig.savefig("travel_time_threshold.pdf", bbox_inches='tight')
